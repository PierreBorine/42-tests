...
norminette "$ex_path/ft_strs_to_tab.c" -R CheckForbiddenSourceHeader

# Compile everything
tempdir="$(mktemp -d)"
cp $SCRIPT_PATH/C08/ex04/main.c $tempdir/main.c
cp $SCRIPT_PATH/C08/ex04/head.h $tempdir/ft_stock_str.h
cp "$ex_path/ft_strs_to_tab.c" $tempdir/
executable="$tempdir/a.out"
$compile_cmd $tempdir/main.c $tempdir/ft_strs_to_tab.c -o $executable
...
$executable "Hello" " " "" "How" "are" "dOINg"
